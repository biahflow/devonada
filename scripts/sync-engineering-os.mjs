 ...
 * ## Por que existe
 *
 * `AGENTS.md`, `CLAUDE.md`, `README.md` e `docs/agent-guidelines.md` mandavam carregar a
 * camada global de `~/workspace/engineeringOS/` — um caminho absoluto da máquina de uma
 * pessoa, escrito em quatro arquivos de instrução viva. O CI nunca alcançou esse caminho;
 * um colaborador novo nunca alcançou; um agente em nuvem nunca alcançou. E no dia em que
 * o diretório mudou de lugar, ele parou de alcançar também para quem o escreveu — em
 * silêncio, porque um import que não resolve não é um erro, é uma ausência.
 *
 * O `docs/engineering-os-adoption.md` declarava `ENGINEERING_OS_COMPLIANT` o tempo todo.
 * Uma regra que só um executor enxerga não é regra do repositório: é contexto privado de
 * uma sessão.
 *
 * ## As três decisões
 *
 ...
 * `scripts/eos-sync.test.js` exercita. A lista de arquivos sai de
...
function origin() {
  return process.env.DEVONADA_EOS_ORIGIN || DEFAULT_ORIGIN;
}
