...
import { CardTino } from './CardTino';
import { umResumo } from '../../test/mocks';

/**
 * O card do Tino é onde o custo diário dos juros chega ao usuário. O número em
...
describe('CardTino', () => {
  it('mostra a ação sugerida e o CTA', () => {
    render(<CardTino resumo={umResumo()} />);
  ...
  it('sem o campo, o card fica igual ao que era — e nunca inventa R$ 0,00 por dia', () => {
    const { toJSON } = render(<CardTino resumo={umResumo()} />);
  ...
  it('campo zerado também não vira frase', () => {
    const { toJSON } = render(
      <CardTino resumo={umResumo({ custoDiarioJuros: 0, quantidadeDividasSemTaxa: 0 })} />,
    );
    expect(JSON.stringify(toJSON())).not.toContain('por dia');
  });
});

describe('copy do card do Tino', () => {
